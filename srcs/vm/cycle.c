...
/*   Updated: 2020/09/14 15:32:07 by joris         ########   odam.nl         */
/*   Updated: 2020/09/14 17:25:59 by merlijn       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static void		vm_run_cursor(t_arena *arena_s, t_cursor *current)
{
	...
	if (is_opcode(current->opcode))
	{
		...
		{
			ft_memcpy(&enbyte, &arena_s->mem[get_pos(current->pos, 1)],
			sizeof(t_enbyte));
			reverse_eb(&enbyte);
		}
		if ((!get_opinfo(current->opcode)->has_enbyte ||
			(get_opinfo(current->opcode)->has_enbyte &&
			is_valid_enbyte(current->opcode, enbyte))) &&
