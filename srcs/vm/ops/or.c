...
static int	get_or_argument_values(char *mem, t_cursor *cursor)
{
	int offset;
...
void		inst_or(t_arena *arena, t_cursor *cursor)
{
	...
	if (check_or_argument_types(cursor->args) == kError)
		return ;
	if (get_or_argument_values(arena->mem, cursor) == kError)
		return ;
